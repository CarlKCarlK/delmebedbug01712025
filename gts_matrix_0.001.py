...
def read_snp_ids_from_bed(bed_filename):
    """
    Given a .bed filename, derive the corresponding .bim filename,
    read its second column (SNP IDs), and return them as a list.

    :param bed_filename: Path to the .bed file (e.g., "mydata.bed")
    :return: List of SNP IDs exactly as they appear in the .bim file
    """
    # 1. Derive the .bim filename
    #    Remove the .bed suffix and replace with .bim
    #    e.g., "mydata.bed" -> "mydata.bim"
    base, ext = os.path.splitext(bed_filename)
    if ext.lower() != ".bed":
        raise ValueError(f"Expected .bed extension, got '{ext}' instead.")
    bim_filename = base + ".bim"

    # 2. Read the .bim file directly
    snp_ids = []
    with open(bim_filename, "rt", encoding="utf-8") as f:
        for line in f:
            # .bim format has 6 columns: [chrom, snp_id, dist, pos, allele1, allele2]
            fields = line.strip().split()
            snp_ids.append(fields[1])  # second column is SNP ID
...
def gts_matrix_chromosome(chrom_out, pedigree, outprefix, bedfile=None, bgenfile=None, par_gts_f=None,
                        fit_sib=False, parsum=False, max_missing=5, min_maf=0.001):
    #ipdb.set_trace()
    ...
    if bedfile is not None:
        ...
        snp_ids = read_snp_ids_from_bed(bedfile)
        snp_ids_II = bed.sid
...
parser.add_argument('--min_maf',type=float,help='Ignore SNPs with minor allele frequency below min_maf (default 0.01)', default=0.001)
...
def main(args):
    ...
    for i in range(chroms.shape[0]):
        ...
        gts_matrix_chromosome(chroms[i], ped, args.out, bedfile=bedfiles[i], bgenfile=bgenfiles[i], par_gts_f=pargts_list[i],
                        fit_sib=args.fit_sib, parsum=args.parsum, max_missing=5, min_maf=0.001)
